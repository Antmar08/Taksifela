<!DOCTYPE html>
<html>

<head>
    <link rel="stylesheet" href="../style css/hinnasto.css" />
</head>

<body>

</body>

</html>

<?php

include "../Kirjautuminen/connect.php";

$taksa =                    $_POST['taksa'];
$alv =                      $_POST['alv'];
$esimerkkimatka =           $_POST['esimerkkimatka'];
$aloitusmaksu =             $_POST['aloitusmaksu'];
$aloitusmaksumuina =        $_POST['aloitusmaksumuina'];
$matka =                    $_POST['matka'];
$aika =                     $_POST['aika'];
$id =                       $_POST['id'];

$sql = "UPDATE hinnasto SET taksa='$taksa', alv='$alv', esimerkkimatka='$esimerkkimatka', aloitusmaksu='$aloitusmaksu', aloitusmaksumuina='$aloitusmaksumuina', matka='$matka', aika='$aika' WHERE id='$id'";

if ($conn->query($sql) === TRUE) {
    echo "Hinnaston tiedot muokattu onnistuneesti, <a href='../suomi/hinnasto.php'>Palaa takaisin</a>";

} else {
    echo "Virhe: " . $conn->error;
}
?>